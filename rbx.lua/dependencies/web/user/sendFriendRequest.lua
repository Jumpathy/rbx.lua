...
function module.run(authentication,userId,callback)
	local run = function(userId)
		if(type(userId) == "number" or resolveToNumber(userId) ~= nil) then
			local endpoint = "https://friends.roblox.com/v1/users/"..userId.."/request-friendship";
	...
	if(resolveToNumber(userId) ~= nil or type(userId) == "number") then 
		...
		local response,body = api.request("GET","https://api.roblox.com/users/get-by-username?username="..userId,{},{},authentication,false,false)
		if(response.code == 200) then 
